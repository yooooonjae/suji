...
(function (global) {
  ...
  function line(root, series, opts) {
    ...
    for (const tv of niceTicks(lo, hi, 4)) {
      ...
      el("text", { x: M.l - 7, y: y(tv) + 4, "text-anchor": "end", "font-size": 10.5, fill: css("--ink-3"), "font-family": "var(--font-num)" }, svg)
    ...
    [0, Math.floor((n - 1) / 2), n - 1].forEach(i => {
      if (!lp[i]) return;
      el("text", { x: x(i), y: H - 8, "text-anchor": i === 0 ? "start" : i === n - 1 ? "end" : "middle", "font-size": 10.5, fill: css("--ink-3") }, svg)
  ...
  function smallMultiples(root, data, opts) {
    ...
    names.forEach(name => {
      ...
      el("path", { d, fill: "none", stroke: css("--blueprint-2"), "stroke-width": 1.6 }, svg);
      el("circle", { cx: x(ser.length - 1), cy: y(last), r: 2.4, fill: css("--blueprint") }, svg);
  ...
  function waterfall(root, items, opts) {
    ...
    steps.forEach((s, i) => {
      ...
      // 라벨
      const tx = cx + bw / 2;
      el("text", { x: tx, y: H - 36, "text-anchor": "middle", "font-size": 10.5, fill: css("--ink-2"), "font-weight": isSum ? 800 : 400 }, svg)
        .textContent = s.name;
      el("text", { x: tx, y: H - 22, "text-anchor": "middle", "font-size": 10.5, "font-weight": 700, fill: col, "font-family": "var(--font-num)" }, svg)
  ...
  function tornado(root, items, base, opts) {
    ...
    items.forEach((it, i) => {
      ...
      el("text", { x: xl - 6, y: cy + 4, "text-anchor": "end", "font-size": 10.5, fill: css("--ink-3"), "font-family": "var(--font-num)" }, svg).textContent = fmt.eok(it.low);
      el("text", { x: xr + 6, y: cy + 4, "font-size": 10.5, fill: css("--ink-3"), "font-family": "var(--font-num)" }, svg).textContent = fmt.eok(it.high);
      [neg, pos].forEach(r2 => {
    ...
    el("text", { x: x(base), y: H - 12, "text-anchor": "middle", "font-size": 11, "font-weight": 700, fill: css("--ink-2"), "font-family": "var(--font-num)" }, svg)
  ...
  function heatmap(root, grid, opts) {
    ...
    grid.ys.forEach((yv, r) => {
      ...
      el("text", { x: M.l - 8, y: M.t + r * cellH + cellH / 2 + 4, "text-anchor": "end", "font-size": 10.5, fill: css("--ink-2"), "font-family": "var(--font-num)" }, svg).textContent = yv;
    });
    grid.xs.forEach((xv, c) => {
      el("text", { x: M.l + c * cw + cw / 2, y: H - M.b + 16, "text-anchor": "middle", "font-size": 10.5, fill: css("--ink-2"), "font-family": "var(--font-num)" }, svg).textContent = xv;
    });
    el("text", { x: M.l, y: 16, "font-size": 11, fill: css("--ink-3") }, svg).textContent = (opts.yName || "") + " ↓ / " + (opts.xName || "") + " →   (적색 = 손실)";
    return svg;
  }
  ...
  function gauge(root, value, opts) {
    ...
    el("text", { x: cx, y: cy + 12, "text-anchor": "middle", "font-size": 10.5, fill: css("--ink-3") }, svg).textContent = opts.label || "";
    return svg;
  }
  ...
  function fan(root, hist, fc, opts) {
    ...
    for (const tv of niceTicks(lo, hi, 4)) {
      el("line", { x1: M.l, x2: W - M.r, y1: y(tv), y2: y(tv), stroke: css("--grid"), "stroke-width": 1 }, svg);
      el("text", { x: M.l - 7, y: y(tv) + 4, "text-anchor": "end", "font-size": 10.5, fill: css("--ink-3"), "font-family": "var(--font-num)" }, svg).textContent = fmt.num(tv, 0);
    }
    ...
    // 직접 라벨
    el("text", { x: W - M.r + 5, y: y(fc.median[fc.median.length - 1]) + 4, "font-size": 11, "font-weight": 700, fill: css("--blueprint") }, svg).textContent = "예측";
    el("text", { x: x(h0) - 5, y: M.t + 10, "text-anchor": "end", "font-size": 10.5, fill: css("--ink-3") }, svg).textContent = "실적←";
    // x 라벨
    const labels = hist.map(p => p.label).concat(fc.labels || []);
    [0, h0, n - 1].forEach(i => {
      el("text", { x: x(i), y: H - 8, "text-anchor": i === 0 ? "start" : i === n - 1 ? "end" : "middle", "font-size": 10.5, fill: css("--ink-3") }, svg).textContent = labels[i] || "";
    });
    // 호버
    const hot = el("rect", { x: M.l, y: M.t, width: W - M.l - M.r, height: H - M.t - M.b, fill: "transparent" }, svg);
    hot.addEventListener("mousemove", ev => {
  ...
  function hbars(root, items, opts) {
    opts = opts || {};
    const W = opts.width || 680, rowH = 30;
    const M = { t: 4, r: 76, b: 4, l: opts.labelW || 118 };
    ...
    items.forEach((d, i) => {
      ...
      el("text", { x: M.l - 9, y: cy + rowH / 2 + 4, "text-anchor": "end", "font-size": 11.5, fill: css("--ink-2"), "font-weight": 600 }, svg).textContent = d.name;
      const bar = el("rect", { x: M.l, y: cy + 6, width: Math.max(2, w2), height: rowH - 12, fill: css(opts.color || "--s1"), rx: 2, opacity: .9 }, svg);
      el("text", { x: M.l + Math.max(2, w2) + 7, y: cy + rowH / 2 + 4, "font-size": 11, fill: css("--ink-2"), "font-family": "var(--font-num)", "font-weight": 700 }, svg)
  ...
  function phase(root, pts, opts) {
    opts = opts || {};
    const W = opts.width || 620, H = 460;
    const M = { t: 26, r: 26, b: 40, l: 52 };
    root.innerHTML = "";
    ...
    el("text", { x: W - M.r, y: y(0) - 6, "text-anchor": "end", "font-size": 10.5, fill: css("--ink-3") }, svg).textContent = "미분양 증가 →";
    el("text", { x: x(0) + 6, y: M.t + 10, "font-size": 10.5, fill: css("--ink-3") }, svg).textContent = "↑ 가격 상승";
    const quad = [["확장", M.l + 8, M.t + 16], ["과열·공급과잉", W - M.r - 8, M.t + 16], ["회복", M.l + 8, H - M.b - 8], ["침체", W - M.r - 8, H - M.b - 8]];
    quad.forEach(([t2, tx, ty], i) => el("text", { x: tx, y: ty, "text-anchor": i % 2 ? "end" : "start", "font-size": 11, "font-weight": 800, fill: css("--ink-3"), opacity: .75 }, svg).textContent = t2);
    pts.forEach(p => {
      const c = el("circle", { cx: x(p.x), cy: y(p.y), r: p.name === "전국" ? 7 : 5.5, fill: p.name === "전국" ? css("--seal") : css("--s1"), stroke: css("--surface"), "stroke-width": 2 }, svg);
      el("text", { x: x(p.x) + 8, y: y(p.y) + 4, "font-size": 10.5, "font-weight": 700, fill: css("--ink-2") }, svg).textContent = p.name;
      c.addEventListener("mousemove", ev => tipShow(`<div class="t-title">${p.name}</div>매매지수 YoY <b class="num">${p.y >= 0 ? "+" : ""}${p.y.toFixed(1)}%</b><br>미분양 YoY <b class="num">${p.x >= 0 ? "+" : ""}${p.x.toFixed(0)}%</b>`, ev.clientX, ev.clientY));
